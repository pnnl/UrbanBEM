...
class EPlusOutputReports(dict):
    ...
    def __init__(self, path):
        ...
        self.delimeter = df[
            df[0].str.strip() == "Tabular Output Report in Format:"
        ].iloc[0, 1]
        ...
        while len(reportDelimeterIndices) > 1:
            report = df.loc[
                reportDelimeterIndices.pop(0) : reportDelimeterIndices[0]
            ].copy()
            self[report.iloc[1, 1], report.iloc[2, 1]] = EPlusReport(
                report.iloc[3:-1].copy()
            )
