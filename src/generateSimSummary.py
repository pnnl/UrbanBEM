if __name__ == "__main__":
    ...
    validCases = [
        case
        for case in tqdm(range(1, 6721), desc="getting successful case numbers")
        if any(
            [
                f"cbecs{case}." in filename
                for filename in listdir("../ep_output/output/")
            ]
        )
    ]
    ...
    validCases = [
        caseID
        for caseID in validCases
        if not open(f"../ep_output/stderr/cbecs{caseID}_err.txt", "r").read()
    ]
            ...
            zip(
                ...
                tqdm(
                    executor.map(
                        EPlusOutputReports,
                        [
                            f"../ep_output/output/cbecs{caseID}.table.csv"
                            for caseID in validCases
                        ],
                    ),
    ...
    modelCheckKey = pd.read_csv("../resources/model_check-2020-07-27.csv", index_col=0)

    # Use the model check key to build the simulation summary dataframe
    simSummary = pd.DataFrame(
        {
            parameter: [
                tables[caseID][row["Report"], row["For"]][row["Table"]].loc[
                    row["Row"], row["Column"]
                ]
